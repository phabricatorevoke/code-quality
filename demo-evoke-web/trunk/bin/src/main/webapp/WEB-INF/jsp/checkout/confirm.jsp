...
<dsp:page>
  ...
  <dsp:importbean bean="/com/evoke/paypal/droplet/PayPalConfirmOrder"/>
  
  <c:set var="stage" value="confirm"/>
	<dsp:getvalueof var="pptoken" param="token" />

	<c:if test="${!empty pptoken}">
		<!-- Execute only if the call is from paypal - check for token. -->
	<dsp:droplet name="PayPalConfirmOrder">
		<dsp:param name="token" param="token" />
		<dsp:param name="authorizedOrder" value="true" />
	</dsp:droplet>
	</c:if>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/checkout/confirm.jsp#2 $$Change: 713816 $--%>
