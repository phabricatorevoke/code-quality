  ...
  <c:choose>
    ...
    <c:otherwise>
      ...
      <c:forEach var="commerceItemRelationship" items="${commerceItemRelationships}" varStatus="status">
        ...
        <c:if test="${commerceItemClassType != 'giftWrapCommerceItem'}">
          <%--
            This droplet generates price beans for a specified product row, that is for a ShippingGroupCommerceItemRelationship.
            These price beans will be used later, when rendering item's price.

            Input parameters:
              relationship
                Specifies a ShippingGroup-CommerceItem relationship. Its commerce items will be used to determine applied discounts.

            Output parameters:
              priceBeansAmount
                Total amount for a relationship specified.
              priceBeansQuantity
                Total relationship's quantity.
              priceBeans
                Price beans for a relationship specified.

            Open parameters:
              output
                Rendered when price beans are ready.
          --%>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/global/gadgets/orderItems.jsp#2 $$Change: 713816 $--%>