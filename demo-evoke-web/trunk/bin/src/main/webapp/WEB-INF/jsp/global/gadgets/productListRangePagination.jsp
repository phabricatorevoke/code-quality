...
<dsp:page>
  ...
  <dsp:getvalueof var="originatingContextPath" bean="/OriginatingRequest.contextPath"/> 
  ...
  <c:if test="${size > arraySplitSize}">
    ...
    <crs:pagination size="${size}" arraySplitSize="${arraySplitSize}" start="${start}" viewAll="${viewAll}" top="${top}">
      ...
      <jsp:attribute name="pageLinkRenderer"> <%-- RENDER PAGE LINKS --%>   
        <%-- Build links href --%>
        <c:set var="url" value="${originatingContextPath}${actionPath}"/>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/global/gadgets/productListRangePagination.jsp#7 $$Change: 739573 $ --%>
