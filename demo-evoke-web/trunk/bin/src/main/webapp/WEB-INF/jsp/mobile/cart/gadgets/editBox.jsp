    ...
    <%-- Show product detail page link and the share button only if there is a URL --%>
		<c:if test="${not empty productUrl}">
			<dsp:a href="javascript:void(0);" iclass="productDetailPageLink">
				...
				<span class="qty"> <fmt:message key="mobile.common.quantity"
						var="quantityAbbrExpansion" /> <abbr
					title="${quantityAbbrExpansion}"><fmt:message
							key="mobile.common.qty" /></abbr>
				</span>
				<span class="qty icon-ArrowLeft"> <c:choose>
			...
			<fmt:message key="mobile.button.share" var="shareTitle" />
			<a href="javascript:void(0);" title="${shareTitle}"
				class="shareLink icon-Share"></a>
		</c:if>

    <%-- Show the delete button even if there is no URL. This will enable users
         to delete free gifts that are part of the order --%>
		<fmt:message key="mobile.button.removeText" var="removeTitle"/>
    <a href="javascript:void(0);" title="${removeTitle}" class="moveLink icon-Remove"></a>
  </div>
</dsp:page>
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/mobile/cart/gadgets/editBox.jsp#4 $$Change: 731411 $--%>
