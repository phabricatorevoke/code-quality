<%--
  ...
  Required parameters:
    ...
    checkoutLogin
      ...
      inputs to the "CheckoutProfileFormHandler". Otherwise, "B2CProfileFormHandler" FormHandler is used.
...
<dsp:page>
  ...
  <dsp:form action="${pageContext.request.requestURI}" method="post" formid="loginPage">
      ...
      <ul class="dataList" role="presentation">
        ...
        <li class="${liClassEmail}">
            <div class="content">
              ...
              <fmt:message var="emailPlaceholder" key="mobile.common.loginEmailAddress"/>
        ...
        <li ${not empty errorMap['email'] || not empty errorMap['badPassword'] || not empty errorMap['password'] || passwordSent == 'true' ? 'class="errorState"' : ''}>
            <div class="content">
              <fmt:message var="password" key="mobile.common.loginPassword"/>
        ...
        <c:if test="${not empty errorMap['email'] || not empty errorMap['badPassword']}">
          <li class="errorStateLine icon-ArrowForget">
            <dsp:a page="${mobileStorePrefix}/myaccount/passwordReset.jsp?email=${emailValue}&checkoutLogin=${checkoutLogin}" title="${forgotPasswordTitle}">
              <span class="content">
                <span class="errorMessageLine"><fmt:message key="mobile.button.passwordResetText"/></span>
              </span>
            </dsp:a>
          </li>
        </c:if>
    ...
    <div class="centralButton">
      <fmt:message key="mobile.myaccount_login.submit" var="submitBtnValue"/>
      <dsp:input bean="${loginField}" class="mainActionButton" type="submit" value="${submitBtnValue}"/>
    </div>
  </dsp:form>
</dsp:page>
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/mobile/global/gadgets/loginPage.jsp#4 $$Change: 738081 $--%>
