<%-- 
  ...
  whether a user is anonymous or not. When there are items in the cart, a number of CartFormHandler 
...
<dsp:page>
  ...
  <c:choose>
    ...
    <c:otherwise>
      ...
      <%--
        Check if the shopping cart contains unselected gifts.
        If yes, redirect to the intermediate page where we offer
        to select gifts. 
       --%>
      ...
      <dsp:input type="hidden" bean="CartFormHandler.expressCheckoutErrorURL"
                 value="${originatingRequest.requestURI}"/>
      <dsp:input type="hidden" bean="CartFormHandler.removeItemFromOrderSuccessURL" 
                 value="${originatingRequest.requestURI}"/>
      <dsp:input type="hidden" bean="CartFormHandler.removeItemFromOrderErrorURL" 
                 value="${originatingRequest.requestURI}"/>
      ...
      <%--
       ...
       Pass 'expressCheckout' parameter for Express Checkout handling and
       'loginDuringCheckout' if user is not logged in and need to be authorized
       before checkout. 
      --%>
      <dsp:droplet name="GiftWithPurchaseSelectionsDroplet">
        <dsp:param name="order" bean="ShoppingCart.current"/>
        ...
        <dsp:oparam name="output">
          <dsp:input type="hidden" bean="CartFormHandler.shippingInfoURL" 
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/cart/gadgets/cartContents.jsp#1 $$Change: 713790 $--%>