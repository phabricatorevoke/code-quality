<%-- 
  ...
  Optional Parameters:
    navigable
      boolean value that defines whether an item can be added to the wishlist or not.
    selection
    ...
    gwpCount
      ...
      markers can be rendered on several lines, as we render non-gwp qunatity of commerce item and every
    ...
    <c:when test='${auxiliaryDataType == "sampleSku"}'>
      <li>
       <dsp:input iclass="atg_store_textButton atg_store_actionDelete" type="submit" name="remove_ci_${nameSuffix}"
                   bean="CartFormHandler.removeItemFromOrder" value="${deleteButtonTitle}" 
        ...
        <c:choose>
          ...
          <c:when test="${not empty selection && empty missedQty}">
            <dsp:input iclass="atg_store_textButton atg_store_actionDelete" type="submit" name="remove_gwp_${nameSuffix}" 
                       bean="CartFormHandler.removeItemGwp" value="${deleteButtonTitle}" 
                       submitvalue="${currentItemId}"/>
          </c:when>
          ...
          <c:when test="${not empty missedQty}">
            <dsp:input type="hidden" converter="map"
                      bean="CartFormHandler.itemNonGwpQuantities" value="${currentItemId}=${missedQty}"/>
            <dsp:input iclass="atg_store_textButton atg_store_actionDelete" type="submit" name="remove_non_gwp_${nameSuffix}"
                       bean="CartFormHandler.removeItemNonGwp" value="${deleteButtonTitle}"
                       submitvalue="${currentItemId}"/>
          </c:when>
          ...
          <c:otherwise>
            <%--
              You should never use unique IDs in form element names
              or a memory leak will result. We are using 'count' for names
              to so DropletEventServlet can reuse entries instead of 
              creating new ones (which is happen when name contains ${commerceItem.id} 
              which is unique and cannot be reused)
            --%>
            <dsp:input iclass="atg_store_textButton atg_store_actionDelete" type="submit" name="remove_ci_${nameSuffix}"
                       bean="CartFormHandler[${count}].removeItemFromOrder" value="${deleteButtonTitle}" 
                       submitvalue="${currentItemId}"/>
          </c:otherwise>
        </c:choose>
      </li>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/cart/gadgets/itemListingButtons.jsp#2 $$Change: 713816 $--%>
