<%--
  ...
  It allows to use existing credit card or create new card. When creating new card, the user can choose an existing address or create new one.
...
<dsp:page>
  ...
  <%--
    ...
    Input Parameters:
      createOneInfoPerUnit - Creates one CommerceItemShippingInfo for each individual unit
                             contained by a commerce item
      clear - clear both the CommerceItemShippingInfoContainer and the ShippingGroupMapContainer
      shippingGroupTypes - Comma separated list of ShippingGroup types used to determine which 
                           ShippingGroupInitializer components are executed
      initShippingGroups - ShippingGroup types will be initialized
      initBasedOnOrder - create a CommerceItemShippingInfo for each 
  ...
  <div class="atg_store_checkoutOption" id="atg_store_checkoutOptionArea">
    ...
    <c:if test="${not empty shippingGroupMap}">
      <%-- 
        ...
        Input parameters:
          defaultKey - The parameter that defines the map key of the default item that should be placed
                       in the beginning of the array.   
          sortByKeys - Boolean that specifies whether to sort map entries by keys or not.
          map - The parameter that defines the map of items to convert to the sorted array.          
             
        Open parameters:
          output - Rendered for permitted shipping address list, permittedAddresses parameter contains 
                   the permitted shipping addresses.
          empty - Rendered if there are no shipping addresses or there are no permitted shipping
                  addresses  
        
        Output parameters:
          permittedAddresses - Contains the permitted shipping addresses  
      --%>  
    ...
    <div id="atg_store_chooseCardAddress" class="${!empty permittedAddresses?'atg_store_existingAddresses ':''}">
          ...
          <fieldset class="atg_store_billingAddresses">
            ...
            <div class="atg_store_saveSelectAddress atg_store_formActions">
              <span class="atg_store_basicButton">
                ...
                <dsp:input type="submit" bean="BillingFormHandler.billingWithSavedAddressAndNewCard" value="${continueButtonText}"
      ...
      <div id="atg_store_enterNewBillingAddress">
        ...
        <div class="atg_store_saveNewBillingAddress atg_store_formActions">
          <span class="atg_store_basicButton">
            <dsp:input type="submit" bean="BillingFormHandler.billingWithNewAddressAndNewCard" value="${reviewOrderButtonText}"
      ...
      <div class="atg_store_formFooter">
        ...
        <dsp:droplet name="ProfileSecurityStatus">
          <dsp:oparam name="anonymous">
            <dsp:input type="hidden" bean="BillingFormHandler.sessionExpirationURL" value="${pageContext.request.contextPath}/index.jsp"/>
          </dsp:oparam>
          <dsp:oparam name="default">
            <dsp:input type="hidden" bean="BillingFormHandler.sessionExpirationURL" value="${pageContext.request.contextPath}/checkout/login.jsp"/>
          </dsp:oparam>
        </dsp:droplet>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/checkout/gadgets/billingForm.jsp#1 $$Change: 713790 $--%>
