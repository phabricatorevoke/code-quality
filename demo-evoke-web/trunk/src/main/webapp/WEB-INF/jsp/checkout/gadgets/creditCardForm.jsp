<%--
  This gadget renders input fields, necessary to create new credit card. It should be used to render the billing page.
...
<dsp:page>
  ...
  <ul class="atg_store_basicForm atg_store_addNewCreditCard">
    ...
    <li class="atg_store_ccCsvCode">
      <dsp:getvalueof var="requireCreditCardVerification" vartype="java.lang.Boolean" bean="StoreConfiguration.requireCreditCardVerification"/>
      <c:if test="${requireCreditCardVerification}">
        ...
        <dsp:input bean="BillingFormHandler.newCreditCardVerificationNumber" type="text"
                   name="atg_store_verificationNumberInput" id="atg_store_verificationNumberInput" autocomplete="off"/>

        <fmt:message var="whatisThisTitle" key="checkout_billing.whatIsThis"/>
        <a href="${pageContext.request.contextPath}/checkout/whatsThisPopup.jsp" title="${whatisThisTitle}" class="atg_store_help" target="popup">
          ${whatisThisTitle}
        </a>
      </c:if>
    </li>
    ...
    <li class="atg_store_saveCC option" style="${transient ? 'display: none;' : ''}">
      ...
      <dsp:input type="checkbox" bean="BillingFormHandler.saveCreditCard" name="addCard" iclass="checkbox" checked="${!transient}"
                 id="atg_store_ccAddSaveccInput"/> <label for="atg_store_ccAddSaveccInput"><fmt:message key="checkout_billing.savePaymentInfor"/></label>
    </li>
  </ul>
</dsp:page>
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/10.1.2/Storefront/j2ee/store.war/checkout/gadgets/creditCardForm.jsp#2 $$Change: 713816 $--%>
