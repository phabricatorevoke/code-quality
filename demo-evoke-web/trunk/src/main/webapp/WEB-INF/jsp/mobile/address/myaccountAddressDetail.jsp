...
<dsp:page>
  ...
  <c:set var="addrPropertyName">
    <c:choose>
      <c:when test="${cardOper == 'add' && addrOper == 'add'}">/atg/userprofiling/MobileB2CProfileFormHandler.billAddrValue</c:when>
      <c:otherwise>/atg/userprofiling/MobileB2CProfileFormHandler.editValue</c:otherwise>
    </c:choose>
  </c:set>
              ...
              <div class="moveItems">
                <ul class="dataList">
                  <li class="remove">
                    ...
                    <dsp:a title="${deleteText}" bean="MobileB2CProfileFormHandler.removeAddress" value="${editAddrNickname}"
                           href="addressBook.jsp" id="removeLink" iclass="icon-Remove">
                      <fmt:message key="mobile.button.removeText"/>
                    </dsp:a>
                  </li>
                </ul>
              </div>
    ...
    <jsp:body>
      ...
      <div class="dataContainer">
        ...
        <dsp:form action="${pageContext.request.requestURI}" method="post">
          ...
          <dsp:input type="hidden" bean="MobileB2CProfileFormHandler.successURL" value="${successURL}"/>
          <dsp:input type="hidden" bean="MobileB2CProfileFormHandler.errorURL" value="${errorURL}"/>
                ...
                <div class="content">
                  <div class="checkBox">
                    <dsp:input type="checkbox" name="useShippingAddressAsDefault" id="addresses_useShippingAddressAsDefault"
                               bean="MobileB2CProfileFormHandler.useShippingAddressAsDefault"
          ...
          <c:if test="${addrType == 'billing' && addrOper == 'edit' && cardOper == 'edit'}">
            ...
            <dsp:getvalueof var="defaultAddressId" bean="Profile.shippingAddress.repositoryId"/>
            <dsp:getvalueof var="currentAddressId" bean="${addrPropertyName}.addressId"/>
            <dsp:input type="hidden" bean="MobileB2CProfileFormHandler.useShippingAddressAsDefault"
          ...
          <div class="centralButton">
            <c:set var="submitBtnHandleMethod">
              <c:if test="${addrType == 'shipping' && addrOper == 'edit'}">MobileB2CProfileFormHandler.updateAddress</c:if>
              <c:if test="${addrType == 'shipping' && addrOper == 'add'}">MobileB2CProfileFormHandler.newAddress</c:if>
              <c:if test="${addrType == 'billing'  && cardOper == 'add'  && addrOper == 'add'}">MobileB2CProfileFormHandler.createNewCreditCardAndAddress</c:if>
              <c:if test="${addrType == 'billing'  && cardOper == 'add'  && addrOper == 'edit'}">MobileB2CProfileFormHandler.updateAddress</c:if>
              <c:if test="${addrType == 'billing'  && cardOper == 'edit' && addrOper == 'add'}">MobileB2CProfileFormHandler.newAddress</c:if>
              <c:if test="${addrType == 'billing'  && cardOper == 'edit' && addrOper == 'edit'}">MobileB2CProfileFormHandler.updateAddress</c:if>
            </c:set>
