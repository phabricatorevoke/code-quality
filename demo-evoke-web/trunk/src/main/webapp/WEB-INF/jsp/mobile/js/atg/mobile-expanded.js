...
CRSMA.cart = function() {
  ...
  /**
   * Initialize the "Featured Items" slider panel.
   * @public
   */
  var initFeaturedItemsSliderPanel = function() {
    CRSMA.sliders.createSlider({
      gridid                : "#featuredItemsContainer",
  ...
  /**
   * Initialize the "Recently Viewed Items" slider panel.
   * @public
   */
  var initRecentlyViewedItemsSliderPanel = function() {
    CRSMA.sliders.createSlider({
      gridid                : "#recentlyViewedItemsContainer",
      onTouchEventException : function(el) {
        $(el).css({
          "height"    : "70px",
          "overflow-x": "auto",
  ...
  return {
    ...
    "initFeaturedItemsSliderPanel"       : initFeaturedItemsSliderPanel,
    "initRecentlyViewedItemsSliderPanel" : initRecentlyViewedItemsSliderPanel
  };
}(); // END CRSMA.cart
/**
...
CRSMA.global = function() {
  ...
  var initMobilePage = function() {
    ...
    var isBrowseRequest = (params["Ntt"] == null);

    var resultsListCount = 0;
    var searchResults = $("div.searchResults");
    if (searchResults.length > 0) {
      $.extend(searchResults, {dataAttribute: CRSMA.global.dataAccessor});
      resultsListCount = searchResults.dataAttribute("resultsListCount");
    }
    ...
    if (guidedNavExists) {
      $.extend(guidedNav, {dataAttribute: CRSMA.global.dataAccessor});
      navGroupsCount = guidedNav.dataAttribute("navigationGroupsCount");
    }
    ...
    CRSMA.global.registerStates([{
      // ResultList
      id: "main",
      buttonTitle: (isBrowseRequest ? getBrowseCategoryName() : getSearchTerm()),
      buttonNavigation: CRSMA.i18n.getMessage("mobile.js.search.refine"),
      active: isMainActive
    },{
      // Breadcrumbs, GuidedNavigation
      id: "secondary",
      buttonTitle: (guidedNavExists ? (isBrowseRequest ? CRSMA.i18n.getMessage("mobile.js.search.shop.by") : CRSMA.i18n.getMessage("mobile.js.search.refine.by")) : ""),
   ...
   * @private
   */
  var getBrowseCategoryName = function() {
  ...
  var focusOnFirstInputInErrorState = function() {
    $('.errorState .content input')[0].focus();
  };
...
CRSMA.home = function() {
  ...
  /**
   * Initializes the home page. <br/><br/>
   *
   * First, store the promotional content items in case we need to re-render them
   * based on orientation changes. <br/><br/>
   *
   * Then, register the templates for product info JSON. Then just to display the product info
   * from session storage, if present, or we down the product JSON, store it, and display it.
   *
...
CRSMA.product = function() {
  ...
  /** 
   * Initializes CRSMA.product context variables, calls {@link CRSMA.product-initRelatedItemsSlider}.<br/>
   ...
   * @public
   */
  var initRelatedItemsSlider = function() {
    CRSMA.sliders.createSlider({
...
CRSMA.search = function() {
  ...
  var initSearch = function() {
    if (!isSearchInit) {
      ...
      $multisiteContainer = $("#searchMultisiteContainer");
      $searchField.bind("keyup", searchKeyUp);
      $("#searchForm").submit(function () {
        var isEmptyField = isSearchFieldEmpty();
        if (isEmptyField) {setTimeout(function() {$multisiteContainer.show()}, 5);}
        return !isEmptyField;
      });
      $(document).bind("touchend mouseup", documentTouchEnd);
      isSearchInit = true;
    }
  }

  /**
   * Hides "Multisite" menu when user types something in "Search" field.
   * @private
   */
  var searchKeyUp = function() {
    $multisiteContainer.hide();
    performClearButton();
  };
  
  /**
   * Hides/shows "Clear search field" button.
   * @private
   */
  var performClearButton = function() {
    if (!isSearchFieldEmpty()) {
      // If button is not added yet - add button
      // setTimeout used to avoid setting phantom focus on button
      if ($(".clearSearchBtn").length == 0) {
        setTimeout(function() {
          $(".searchBar").after("<span class='clearSearchBtn' onclick='CRSMA.search.clearSearchField()'></span>");
        }, 100);
      }
    } else {
  ...
  /**
   * Checks if "Multisite" menu should be hidden or no.
   * @private
   */
  var documentTouchEnd = function(event) {
    if (searchBlurTimeoutId) {
      clearTimeout(searchBlurTimeoutId);
    }
    var $target = $(event.target);
    var inMultisite = false;
    // When user clicks/touches inside the "Multisite" menu,
    // it shouldn't be hidden. When outside - hide it
    $target.parents().andSelf().each(function (i) {
      if ($(this).attr('id') == $multisiteContainer.attr('id')) {
        inMultisite = true;
        return false;
      }
    });
    if (!inMultisite && $target.attr('id') != 'searchText') {
      hideMultisite();
    }
  };

  /**
   * Hides SitePicker and sets "Search" field to it's initial state.
   * @private
   */
  var hideMultisite = function() {
    $multisiteContainer.hide();
  ...
  var searchOnBlur = function() {
    searchBlurTimeoutId = setTimeout(function() {
      hideMultisite();
      $(".clearSearchBtn").remove();
    }, 300);
  };
...
/**
 * @namespace "Sliders" Javascript Module of "Commerce Reference Store Mobile Application"
 * @description Holds functionality related to sliders. <br>
 * It's based on <a href="https://github.com/zgrossbart/jstouchslide">"touchslider.js"</a> plugin code, that was adjusted for CRSMA needs.<br><br>
 * 
...
CRSMA.sliders = function() {
  ...
  var baseSlider = {
      ...
      $(pGridId).each(function() {
        ...
        this.ontouchend = function(pEvent) {
          ...
          if (sliderObject.sliding) {
            ...
            /* We never slid so we can just return true and perform the default touch end */
            return true;
          }
        };
    ...
    /**
     * A little helper to cut off the 'px' at the end of the "left"
     * CSS attribute and parse the result as a number.
     *
    ...
    touchEnd: function(/*JQuery*/p$Elem, /*event*/pEvent, /*int*/pTouchSensitivity, /*function*/pCallBack) {
      if (this.getLeft(p$Elem) > 0) {
        ...
        this.doSlide(p$Elem, 0, "2s", false);
        ...
        if ((this.width - p$Elem.parent().width()) > 0) {
          this.doSlide(p$Elem, "-" + (this.width - p$Elem.parent().width()), "2s", false);
        } else {
          this.doSlide(p$Elem, 0, "2s", false);
        }
        p$Elem.parent().removeClass("sliding");
    ...
    slideMomentum: function(/*jQuery*/p$Elem, /*event*/pEvent, /*int*/pTouchSensitivity, /*function*/pCallBack) {
      ...
      */
      ...
      if (this.slidingLeft) {
        ...
        this.doSlide(p$Elem, newLeftValue, "0.5s", true, pCallBack);
      } else {
     ...
     * @param {boolean} pShowNext  
     * @param pCallBack function
     * @public 
     */
    doSlide: function(/*jQuery*/p$Elem, /*int*/pX, /*string*/pDuration, /*boolean*/pShowNext, /*function*/pCallBack) {
      p$Elem.css({
        left: pX + "px",
  ...
  /**
   * Creates slider with the specified options.<br><br>
   * 
   ...
   *   <li><b>touchSensivity</b> - number (3000 by default)</li>
  ...
  var createSlider = function(options){
    ...
    sliderClass.createSlidePanel(
        settings.gridid, settings.cellPrefix, settings.numberOfCells, 
        settings.touchSensivity, settings.onTouchEventException);
    return sliderClass;
  };
